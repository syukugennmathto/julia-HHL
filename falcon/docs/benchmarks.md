...
| 署名 2.03 ms | ms | 検証 0.564 ms | ms |
|:---|---:|:---|---:|
| `sample_preimage` | 1.386 | `polymulq` | 0.161 |
| **`sqnorm`** | **0.293** | **`sqnorm`** | **0.258** |
...
**検証の 46% が `sqnorm` 1 つ**だった。扱う係数は `|c| ≤ 6144` で
総和は `2^38` 未満 ― 大きい数だから遅いのではなく `BigInt` だから遅い。
`sqnorm` は #034 で 227 倍速くした（範囲チェック + `Int128`）。

**サンプラは律速ではない**（`sample_preimage` の 10%）。
