...
function main()
    ...
    for j in sort(collect(keys(sa)))
        ...
        a, b, gc, fc = r
        nrec += 1
        # Is it a USABLE key?  Two checks the attacker can make without the
        # secret: the public-key relation (already imposed inside recover) and
        # the norm bound key generation itself enforces.
        nrm = sum(x -> x * x, gc) + sum(x -> x * x, fc)
        bound = 1.17^2 * q
        # and, for the record, which NTRU symmetry x^k (f,g) it landed on
        sym = symmetry(fc, gc, truef, trueg)
        @printf("-> *** KEY RECOVERED *** (a,b)=(%2d,%2d)  ||(g,-f)||^2 = %d %s bound %d  %s\n",
                a, b, nrm, nrm <= bound ? "<=" : ">", round(Int, bound), sym)
    end
    println()
    @printf("# %d divergent pairs; %d yielded the private key by section 5.1,\n", ntried, nrec)
