...
|:---|---:|---:|---:|---:|
| `keygen` | 13.0 | 6.35 | **6766** | 520x |
| `expand_privkey` | 1.77 | 0.081 | **0.790** | **0.4x** |
| `sign_dyn` | 4.43 | 0.311 | （該当なし） | — |
| `sign_tree` | 2.04 | 0.194 | **2.04** | **1.0x** |
| `verify` | 0.030 | 0.029 | **0.230** | 7.7x |

`sqnorm` を直したあとの数字（#034）。直す前は verify が 0.458 ms（15.3x）だった。
`keygen` の 9610 → 6766 は `sqnorm` の効果ではなく **`ntru_solve` の
再試行回数のばらつき**（3 サンプル、mean 8946 / min 5366）。
鍵生成は `sqnorm` をほとんど呼ばないので、ここは同じ数字の別の引きでしかない。
